...
class AudioAnalysisController:
    ...
    async def _run_background_scan(self) -> None:
        ...
        versions = {p.domain: p.analysis_version for p in providers}
        candidates = await self._find_candidates_missing_analysis(versions, limit=0)
        if not candidates:
            return
    ...
    async def _find_candidates_missing_analysis(
        ...
        """
        Return rows {item_id, provider_instance, missing_domains} for tracks needing analysis.

        A track is a candidate for a domain when it has no row, or its row's
        analysis_version is below that domain's current version, so providers can
        force re-analysis by storing a result at a lower version.

        :param aa_provider_versions: Current analysis_version per AA provider domain.
        ...
        # CROSS JOIN (track x possible domain), keep pairs with no current-version row,
        # GROUP_CONCAT the missing domains per track.
        domains = list(aa_provider_versions)
        fs_inline = ", ".join(f"'{d}'" for d in filesystem_domains)
        aa_select_terms = " UNION ALL ".join(
            f"SELECT :aa_{i} AS aa_provider_domain, :ver_{i} AS analysis_version"
            for i in range(len(domains))
        )
        params: dict[str, Any] = {"media_type": MediaType.TRACK.value}
        for i, domain in enumerate(domains):
            params[f"aa_{i}"] = domain
            params[f"ver_{i}"] = aa_provider_versions[domain]
        query = (
            ...
            f"      AND (aa.analysis_version IS NULL "
            f"           OR aa.analysis_version >= possible.analysis_version)"
            f"  ) "
