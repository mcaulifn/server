...
async def test_find_candidates_handles_sqlite_row_without_get(
    ...
    # a row below the domain's current version no longer covers its track, so it is
    # re-collected as a candidate; the per-domain version is bound for comparison
    call = controller.mass.music.database.get_rows_from_query.await_args
    assert call is not None
    sql, params = call.args[:2]
    assert "analysis_version" in sql
    assert params["ver_0"] == 1
